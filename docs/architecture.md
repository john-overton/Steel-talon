...
- `collideBulletsEnemies(w)` is O(bullets × enemies) using `circlesOverlap`: each hit kills the bullet, decrements enemy hp by `b.dmg`, and spawns a small particle burst; hp reaching 0 kills the enemy, spawns a larger burst plus trailing smoke puffs, and counts as a kill. Returns `{ hits, kills }` so the caller can pick `hit` vs. `explode` SFX. (Splash damage and salvage/score payout arrive with milestone 6.)
